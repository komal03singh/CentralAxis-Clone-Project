...
function Card1({heading,content,ulList=[],videoUrl,vidPosition}) {

  const isVideoLeft = vidPosition === "left";

  return (
    <div className={`flex min-h-96 w-full rounded-lg border border-white/10 bg-[#18181B] ${isVideoLeft ? 'flex-row-reverse' : ''}`}>
        ...
        <ul className='flex flex-col items-start gap-3 pl-2 md:text-lg'>
          {ulList?.map((List,index)=>(
            <li className='flex items-center gap-4 font-normal text-[#A1A1AA]' key={index}>
              <Image className='bg-[#787882] rounded-full' height={24} width={24} src="/tick.svg" alt="" />{List}</li>
            ))}
        </ul>
      </div>
